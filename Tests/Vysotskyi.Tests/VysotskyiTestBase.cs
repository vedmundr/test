...
using Vysotskyi.Migrations.SeedData;
...
namespace Vysotskyi.Tests
{
    public abstract class VysotskyiTestBase : AbpIntegratedTestBase<VysotskyiTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected VysotskyiTestBase()
        {
        ...
        protected void UsingDbContext(Action<VysotskyiDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<VysotskyiDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<VysotskyiDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<VysotskyiDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<VysotskyiDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VysotskyiDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<VysotskyiDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<VysotskyiDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<VysotskyiDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<VysotskyiDbContext, Task<T>> func)
        {
            T result;
